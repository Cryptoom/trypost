...
## Local Test Database (host PHP, no Docker app container)

`compose.yaml` already ships a full Docker dev stack (`app` + `pgsql` + `redis` + `mailpit`), and
its `pgsql` service forwards host port 5432 by default. On a machine where 5432 is already taken
by another project's Postgres container, that default collides and `.env.testing` (which pins
`DB_HOST=127.0.0.1`) points at the wrong database.

`compose.test.yml` is a separate, minimal compose file that starts only a Postgres 16 container
(`pgsql-test`) for the test suite, independent of the main dev stack:

```bash
# start (idempotent, keeps data across restarts via the named volume)
docker compose -f compose.test.yml -p trypost-test up -d

# wait until ready
docker compose -f compose.test.yml -p trypost-test exec pgsql-test pg_isready -U postgres -d trypost

# stop
docker compose -f compose.test.yml -p trypost-test down

# reset (drops the named volume, next start runs docker/postgres-init.sh fresh)
docker compose -f compose.test.yml -p trypost-test down -v
```

- Binds to `127.0.0.1:5433` only, not `0.0.0.0`. Reuses `docker/postgres-init.sh` to create the
  `trypost_test` database (same init script `compose.yaml` uses for its own `pgsql` service).
- `.env.testing` is set to `DB_HOST=127.0.0.1`, `DB_PORT=5433`, `DB_DATABASE=trypost_test`,
  `DB_USERNAME=postgres`, `DB_PASSWORD=password`. This is a **local test credential only**, never
  used outside this container, and the container only accepts connections from localhost.
- `phpunit.xml` sets `DB_CONNECTION`, `DB_DATABASE`, `DB_PASSWORD` via `<php><env>`, which take
  precedence over `.env.testing` because PHPUnit populates `$_ENV`/`$_SERVER` before Laravel boots
  and Dotenv does not overwrite already-set variables. Keep both files' values consistent, or the
  precedence rules above silently pick one over the other.
- **`php artisan test` boots the full console kernel first (before spawning PHPUnit), and that
  first boot uses the default `.env` (or no env file at all if `.env` is absent), not
  `.env.testing`.** On a machine with no `.env` this fails early with unrelated errors (e.g.
  `rtrim(): Argument #1 ($string) must be of type string, null given` in `config/filesystems.php`,
  because `APP_URL` was never loaded). Run the test binary directly instead, which lets PHPUnit's
  `<php><env>` block (including `APP_ENV=testing`) apply before the app boots:
  ```bash
  APP_ENV=testing vendor/bin/pest --compact
  ```
- The image processing tests (Intervention Image / GD) can exceed PHP's default 256M
  `memory_limit` on some local PHP builds. If you see
  `Fatal error: Allowed memory size ... exhausted` in `intervention/image/src/Drivers/Gd/Driver.php`,
  raise it for the run: `APP_ENV=testing php -d memory_limit=1024M vendor/bin/pest --compact`.
- Migrate manually before a first run (or after `down -v`): `php artisan migrate --env=testing --force`.
- The OAuth server (`laravel/passport`, overridden bindings in `app/Passport/*`) needs
  `storage/oauth-private.key` and `storage/oauth-public.key`. Both are gitignored
  (`/storage/*.key`) and not generated by default. Without them, any test that goes through
  `AuthorizationController`/`AccessTokenRepository` fails with `LogicException: Invalid key
  supplied` (`vendor/league/oauth2-server/src/CryptKey.php`). Generate once per worktree:
  ```bash
