...
const platformBrand = (platform: string): string =>
    PLATFORM_BRAND[platform] ?? platform;
const formatLabel = (platform: string): string => FORMAT_LABEL[platform] ?? '';
const platformLogo = (platform: string): string =>
    getPlatformLogo(PLATFORM_LOGO_KEY[platform] ?? platform);
...
const hasActiveSearch = computed(
    () => Boolean(searchQuery.value?.trim()) || Boolean(selectedPlatform.value),
);
...
const applyTemplate = async (template: PostTemplate) => {
    ...
    try {
        const http = useHttp<{ date: string | null }>({
...
<template>
    ...
    <AppLayout>
        <div class="flex h-full flex-1 flex-col gap-6 px-6 py-8">
            <PageHeader
                :title="pageTitle"
                :description="$t('posts.templates.browser_description')"
            />
                        ...
                        >
                            ...
                            >
                                ...
                                <span class="truncate font-semibold">{{
                                    selectedPlatformOption.brand
                                }}</span>
                                <span
                                    class="shrink-0 text-xs font-medium text-foreground/60"
                                    >{{ selectedPlatformOption.format }}</span
                                >
                            </span>
                            <IconChevronDown
                                class="ml-2 size-4 shrink-0 text-foreground/60"
                            />
                        </Button>
                    </PopoverTrigger>
                    <PopoverContent
                    ...
                    >
                        <Command>
                            <CommandInput
                                :placeholder="
                                    $t(
                                        'posts.templates.platform_search_placeholder',
                                    )
                                "
                            />
                            <CommandList>
                                <CommandEmpty>{{
                                    $t('posts.templates.no_platform_match')
                                }}</CommandEmpty>
                                <CommandGroup>
                                    <CommandItem
                                        value="all"
                                        @select="onPlatformChange('all')"
                                    >
                                        <span>{{
                                            $t('posts.templates.all_platforms')
                                        }}</span>
            ...
            <EmptyState
                ...
                :title="
                    hasActiveSearch
                        ? $t('posts.templates.no_search_results')
                        : $t('posts.templates.no_templates')
                "
                :description="
                    hasActiveSearch
                        ? $t('posts.templates.try_different_search')
                        : ''
                "
            />
                    ...
                    >
                        <header class="flex items-center gap-2">
                            ...
                            <span
                                ...
                                >{{ platformBrand(template.platform) }}</span
                            >
                            <Badge
                                variant="warning"
                                class="ml-auto shrink-0 -rotate-2"
                            >
                        ...
                        <h3
                            class="mt-3 text-base leading-snug font-bold text-foreground"
                        >
                            {{ template.name }}
                        </h3>
                        ...
                        <div class="mt-3 flex items-center gap-2">
                            <Badge variant="secondary" class="shrink-0">
                                {{
                                    $t(
                                        `posts.templates.category.${template.category}`,
                                    )
                                }}
                            </Badge>
                            <span
                        ...
                        >
                            <IconLoader2
                                v-if="applyingSlug === template.slug"
                                class="size-4 animate-spin"
                            />
                            {{
                                applyingSlug === template.slug
                                    ? $t('posts.templates.applying')
                                    : $t('posts.templates.use_this')
                            }}
                        </Button>
                    </article>
                </div>
