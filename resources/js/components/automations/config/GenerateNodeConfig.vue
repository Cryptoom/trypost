...
const socialAccounts = computed<SocialAccount[]>(() => {
    const raw = page.props.socialAccounts as
        | { data?: SocialAccount[] }
        | SocialAccount[]
        | undefined;
    if (!raw) return [];
    return Array.isArray(raw)
        ? (raw as SocialAccount[])
        : ((raw as { data: SocialAccount[] }).data ?? []);
});
...
const pinterestBoards = computed<Record<string, PinterestBoardsPayload>>(() => {
    const raw = page.props.pinterestBoards as
        | Record<string, PinterestBoardsPayload>
        | undefined;
    return raw ?? {};
});

const tiktokCreatorInfos = computed<Record<string, TikTokCreatorInfo>>(() => {
    const raw = page.props.tiktokCreatorInfos as
        | Record<string, TikTokCreatorInfo>
        | null
        | undefined;
    return raw ?? {};
});
...
const normalizeAccountsFromData = (): GenerateAccount[] => {
                ...
                content_type:
                    typeof a.content_type === 'string' && a.content_type
                        ...
                        : defaultContentTypeFor(
                              accountById(String(a.social_account_id ?? ''))
                                  ?.platform ?? '',
                          ),
    ...
    if (Array.isArray(legacyIds)) {
        return (legacyIds as string[]).map((id) => ({
            ...
            content_type: defaultContentTypeFor(
                accountById(id)?.platform ?? '',
            ),
            meta: {},
        }));
    }
...
const local = ref<GenerateConfig>({
    ...
    target_slide_count:
        (props.data.target_slide_count as number | undefined) ?? 1,
    prompt_template: (props.data.prompt_template as string) ?? '',
    use_brand_voice:
        (props.data.use_brand_voice as boolean | undefined) ?? true,
    use_brand_visuals:
        (props.data.use_brand_visuals as boolean | undefined) ?? true,
...
const selectedAccountIds = computed(() =>
    local.value.accounts.map((a) => a.social_account_id),
);
...
const toggleAccount = (account: SocialAccount) => {
    if (selectedAccountIds.value.includes(account.id)) {
        local.value.accounts = local.value.accounts.filter(
            (a) => a.social_account_id !== account.id,
        );
        return;
    }
...
const updateContentType = (accountId: string, value: string) => {
    const idx = local.value.accounts.findIndex(
        (a) => a.social_account_id === accountId,
    );
    if (idx === -1) return;
    local.value.accounts[idx] = {
        ...local.value.accounts[idx],
        content_type: value,
    };
};
...
const accountIssue = (accountId: string): string | null => {
    const entry = local.value.accounts.find(
        (a) => a.social_account_id === accountId,
    );
    if (!entry) return null;
    const mediaIssue = getMediaIncompatibilityReason(
        entry.content_type,
        syntheticImages(intendedImageCount.value),
    );
...
const channels = computed<Channel[]>(() =>
    socialAccounts.value.map((account) => {
        ...
        return {
            ...
            contentType:
                entry?.content_type ?? defaultContentTypeFor(account.platform),
            meta: entry?.meta ?? {},
    ...
    <div class="space-y-4">
        <div class="space-y-2">
            <Label class="text-sm font-bold">{{
                $t('automations.config.generate.social_accounts')
            }}</Label>
        ...
        <div class="space-y-2">
            <Label class="text-sm font-bold">{{
                $t('automations.config.generate.style')
            }}</Label>
            <ContentStylePicker
                v-model="local.style"
        ...
        <div v-if="imageCountCap >= 1" class="space-y-2">
            <Label class="text-sm font-bold">{{
                $t('automations.config.generate.image_count')
            }}</Label>
            ...
            <div class="flex flex-wrap gap-2">
                <Button
                    ...
                    size="icon"
                    :variant="
                        local.target_slide_count === n ? 'default' : 'outline'
                    "
        ...
        >
            <div class="space-y-0.5">
                <Label class="text-sm font-bold">{{
                    $t('automations.config.generate.use_brand_voice')
                }}</Label>
                <p class="text-xs text-foreground/60">
                    {{ $t('automations.config.generate.use_brand_voice_hint') }}
                </p>
            </div>
        ...
        <div
            v-show="!editorExpanded && local.target_slide_count >= 1"
            class="flex items-start justify-between gap-3"
        >
            <div class="space-y-0.5">
                <Label class="text-sm font-bold">{{
                    $t('automations.config.generate.use_brand_visuals')
                }}</Label>
                <p class="text-xs text-foreground/60">
                    {{
                        $t('automations.config.generate.use_brand_visuals_hint')
                    }}
                </p>
            </div>
        ...
        <div v-show="!editorExpanded">
            <Label class="mb-1 block">{{
                $t('automations.config.generate.prompt_template')
            }}</Label>
            <div class="h-40">
                <CodeEditor
            ...
            <p class="mt-1 text-xs text-foreground/50">
                {{ $t('automations.config.generate.prompt_template_hint') }}
            </p>
