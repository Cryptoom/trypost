...
| B4-Produktfrage | GEKLAERT, bereits so umgesetzt | Olli: "ja beides oder?" Code (PR #21, `ccb75401`) implementiert bereits BEIDES: Delete raeumt remote via Unpublish auf, DANN lokal (`PostStatusRules::blocksDeletion()` blockt nur noch `Publishing`, nicht mehr `Published`/`PartiallyPublished`), Unpublish bleibt als separate Aktion bestehen. Verifiziert per `git log` gegen `app/Support/PostStatusRules.php` |
| U1-Freigabe | GESTRICHEN 17.09.2026 | Olli explizit: "nein wir wollen nicht helfen!" Kein Upstream-Beitrag |
| Separater Upstream-PR fuer publishStory()-Fix (a30d83c2) | ERLEDIGT 17.09.2026 | [trypostit/trypost#360](https://github.com/trypostit/trypost/pull/360) erstellt, Cherry-Pick von `a30d83c2` auf frischen `upstream/main`-Branch, `php -l` clean, PR offen |
| Upstream-PR #287 Ueberschneidung mit Feature A | GEKLAERT (A0, TPX-02) | Kein Konflikt, PR #287 aendert nur numerische Constraints, keine Migration |
