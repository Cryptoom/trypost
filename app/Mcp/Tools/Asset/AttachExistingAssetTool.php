...
#[Description('Reuse an existing Asset Library item on a post in the current workspace (the workspace bound to this MCP token). Does not upload a new file — for new media use request-media-upload-tool then attach-media-from-upload-tool, or attach-media-from-url-tool. Discover ids with list-assets-tool or get-asset-tool. The post must be draft or scheduled; published, partially published, failed, and publishing posts are rejected. The asset must already be in this workspace library. The asset type must be accepted by every enabled platform on the post (e.g. TikTok-only posts reject images); per-network size, video duration, GIF and MOV caps (see list-content-types-tool) are enforced when the post is scheduled or published. Repeating the same post_id and asset_id does not duplicate the media or change alt. Optional post_platform_ids scopes the asset to specific platforms via the per-platform media selection; omit to keep it available to every enabled platform. Optional alt overrides image alt text only (ignored for video/document), maximum 2000 characters; omit to keep the library item\'s existing alt_text. Requires permission to update the post. Returns the updated post (same shape as other post tools).')]
class AttachExistingAssetTool extends Tool
{
    ...
    public function handle(Request $request): Response|ResponseFactory
    {
        ...
        $validated = $request->validate([
            ...(new AttachExistingAssetRequest)->rules(),
        ...
        PostPlatformMediaScope::apply($post, data_get($validated, 'post_platform_ids', []), [$asset->id]);
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'post_platform_ids' => $schema->array()
                ->items($schema->string())
                ->description('Optional post_platform row UUIDs (from get-post-tool / list-posts-tool) this post already has. When set, the asset is scoped to publish only on these platforms via the per-platform media selection. Omit or pass an empty array to keep today\'s behaviour: the asset is available to every enabled platform. Repeating this call with the same asset_id and post_platform_ids does not duplicate the selection.'),
        ];
    }
}
