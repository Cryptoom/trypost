        ...
        return [
            ...
            'platforms' => $schema->array()
                ->items($schema->object(fn ($p) => [
                    ...
                    'meta' => $p->object()->description('Per-platform metadata override. Instagram/Facebook: aspect_ratio; Instagram collaborators (up to 3 usernames, Reels/feed/carousel only — not Stories; invite must be accepted; live status via GetInstagramCollaboratorsTool). TikTok: privacy_level (required to publish) + flags. Pinterest: board_id (required to publish — call ListPinterestBoardsTool first), title (≤100), link (destination URL). Pin description comes from the post content. Discord: channel_id (required to publish — call ListDiscordChannelsTool first), mentions, embeds. Merged with existing meta.'),
                ]))
